...
import com.li.core.beans.Pram;
import com.li.core.sql.exception.IdTypeException;
...
//	public static final QueryRunner SQLSESSION = new QueryRunner((DataSource)new ClassPathXmlApplicationContext("spring.xml").getBean("dataSource"));
				...
				try {
					...
					if (calssName.equals("int") || calssName.equals("java.lang.Integer")) {
						if (Integer.MAX_VALUE >  new Integer("" + fileValue)) {
							...
							throw new IdTypeException("ID type is not a corresponding type at " + "set" + fileName.substring(0, 1).toUpperCase() + fileName.substring(1) + "\n"
	...
	public static Serializable getNextId(String tableName){
		//SELECT auto_increment FROM information_schema.`TABLES` WHERE TABLE_NAME='li_resource_language';
		try {
